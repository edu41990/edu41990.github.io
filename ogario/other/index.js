/* Url: https://repl.it/@MatheusEduardo3/PROBOT13
 Websocket: wss://PROBOT13-1--MatheusEduardo3.repl.co
----------------------------------------------------------------------------------------
OBSERVAÇÕES:
----------------------------------------------------------------------------------------

1-Maximo de 15 letras por nick;

----------------------------------------------------------------------------------------
2-Evite lotar o server com 199 bots, o jogo fica chato e chegara uma hora que ningué vai ficar jogando, só voce;
----------------------------------------------------------------------------------------
3-Bom jogo;
...
logger.warn(`[PROBOT 2.0.1]`)
logger.good(`[MENSAGEM] Funcionando a versão  ${config.server.version} na porta ${config.server.port}`);
logger.warn(`[NÃO USE BOTS EM EXPERIMENTAL/FFA]`)
            logger.good(`[MENSAGEM] Sem atualizações disponíveis!`)
const game = {
    url: '',
    protocolVersion: 0,
    clientVersion: 0
...
const dataBot = {
    ...
    onclose() {
        if (this.isConnected) {
            this.isConnected = false;
            this.connect();
            logger.error('[MENSAGEM] Bot disconectado!')
        }
    },
    handleBuffer(buffer) {
        ...
        switch (reader.readUint8()) {
            ...
            case 241:
                ...
                logger.good('[MENSAGEM] Bots conectados!');
                break
        }
    }
};
...
new WebSocket.Server({
    ...
    userWS = ws;
    logger.good('[MENSAGEM] Conectado ao Agar.io!');
        ...
        switch (reader.readUint8()) {
            case 0:
                if (!user.startedBots) {
                    ...
                    logger.good('[MENSAGEM] Iniciando bots...')
                }
                break;
            case 1:
                if (user.startedBots && !stoppingBots) {
                    ...
                    setInterval(() => {
                        if (seconds === 20) {
                            ...
                            logger.warn(`[MENSAGEM] Parando bots em ${20 - seconds} seconds`);
                            seconds++
                        }
                    }, 1000)
                }
    ...
    ws.on('close', () => {
        if (user.startedBots && !stoppingBots) {
            ...
            let seconds = 0;
            setInterval(() => {
                if (seconds === 20) process.exit();
                else {
                    logger.warn(`[MENSAGEM] Parando bots em ${20 - seconds} segundos`);
                    seconds++
                }
            }, 1000)
        }
        logger.error('[MENSAGEM] Usuário disconectado!')
    })
});
